...
class FormattedPrice extends React.Component {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        return (
            ...
            nextProps.decimals !== this.props.decimals || !utils.are_equal_shallow(nextState, this.state)
        );
    }
    ...
    render() {
        ...
        let {
            base_asset, quote_asset, base_amount, quote_amount, callPrice,
            marketDirections, marketId, hide_symbols, noPopOver
        } = this.props;
        ...
        let symbols = hide_symbols ? "" :
            (<span data-place="bottom" data-tip={noPopOver && !callPrice ? "Click to invert the price" : null}
                   className={noPopOver && !callPrice ? "clickable inline-block" : ""}
                   onClick={noPopOver && !callPrice ? this.onFlip.bind(this) : null}><AssetName
                name={base_asset.get("symbol")}/>/<AssetName name={quote_asset.get("symbol")}/></span>);

        const currency_popover_body = !noPopOver && !hide_symbols ? (
            <div>
                {!callPrice ?
                    <div className="button" onClick={this.onFlip.bind(this)}><Translate content="exchange.invert"/>
                    </div> : null}
                <div className="button" onClick={this.goToMarket.bind(this)}><Translate content="exchange.to_market"/>
                </div>
            </div>
        ) : null;
