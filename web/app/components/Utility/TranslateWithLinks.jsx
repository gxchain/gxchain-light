...
export default class TranslateWithLinks extends React.Component {
    ...
    linkToFreeDataProduct(prod){
        return <Link to={`/data-market/Free/${prod.id}`}>{prod.product_name}</Link>;
    }

    linkToLeague(league){
        return <Link to={`/data-market/League/${league.id}`}>{league.league_name}</Link>;
    }
        ...
        keys.forEach(key => {
            if (splitText.indexOf(key.arg)) {
                ...
                switch (key.type) {
                    ...
                    case "free_product":
                        value = this.linkToFreeDataProduct(key.value);
                        break;

                    case "league":
                        value = this.linkToLeague(key.value);
                        break;
