...
class Row extends React.Component {
    ...
    render() {
        ...
        if (block > last_irreversible_block_num) {
            pending =
                <span>(<Translate content="operation.pending" blocks={block - last_irreversible_block_num}/>)</span>;
        }
            ...
            <tr>
                {hideOpLabel ? null : (
                    <td style={{width: "20%"}} className="left-td column-hide-tiny">
                        <Link className="inline-block" data-place="bottom"
                              data-tip={counterpart.translate("tooltip.show_block", {block: utils.format_number(this.props.block, 0)})}
                              to={`/block/${this.props.block}`}><TransactionLabel color={color} type={type}/></Link>
                    </td>)}
                <td style={{padding: "8px 5px"}}>
                    ...
                    <div style={{fontSize: 14, paddingTop: 5}}>
                        ...
                        {!this.props.hideFee ?
                            <span className="facolor-fee"> - <FormattedAsset amount={fee.amount} asset={fee.asset_id}/></span> : null}
        ...
        switch (ops[op[0]]) { // For a list of trx types, see chain_types.coffee
            ...
            case 'account_upgrade_merchant':
                column = (
                    <span>
                            <TranslateWithLinks
                                string={"operation.account_upgrade_merchant"}
                                keys={[
                                    {type: "account", value: op[1].account_to_upgrade, arg: "account"}
                                ]}
                            />
