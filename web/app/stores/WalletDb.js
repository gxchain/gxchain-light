...
import {ChainStore, PrivateKey, key, Aes} from "gxbjs/es";
...
class WalletDb extends BaseStore {
    ...
    process_transaction(tr, signer_pubkeys, broadcast, extra_keys = [], no_confirm = false) {
        ...
        return WalletUnlockActions.unlock().then(() => {
            return Promise.all([
                ...
                return tr.get_potential_signatures().then(({pubkeys, addys}) => {
                    ...
                    if (broadcast) {
                        if (this.confirm_transactions&&!no_confirm) {
    ...
    onCreateWallet(password_plaintext,
                   brainkey_plaintext,
                   unlock = false,
                   public_name = "default") {
