...
import {ChainStore, ChainTypes as grapheneChainTypes} from "gxbjs/es";
...
class FeeGroup extends React.Component {
    ...
    render() {
        ...
        let core_asset = ChainStore.getAsset("1.3.1");
        if (!core_asset) {
            return;
        }
        if (core_asset.get("symbol") === 'NULL') {
            core_asset = ChainStore.getAsset("1.3.1");
            if (!core_asset) {
                return;
            }
        }
        ...
        let preferredID = core_asset.get("id");
        ...
        let fees = opIds.map((feeIdx) => {
            ...
            for (let key in fee) {
                ...
                let assetAmount = amount ? <FormattedAsset amount={amount} asset={preferredID}/> : feeTypes["_none"];
                let equivalentAmount = amount ?
                    <EquivalentValueComponent fromAsset={preferredID} fullPrecision={true} amount={amount}
                                              toAsset={preferredUnit}/> : feeTypes["_none"];
                let assetAmountLTM = amount * 0.2 ?
                    <FormattedAsset amount={amount * 0.2} asset={preferredID}/> : feeTypes["_none"];
                let equivalentAmountLTM = amount * 0.2 ?
                    <EquivalentValueComponent fromAsset={preferredID} fullPrecision={true} amount={amount * 0.2}
                                              toAsset={preferredUnit}/> : feeTypes["_none"];
                let title = null;
...
class Fees extends React.Component {
    ...
    render() {
        ...
        for (let groupName in fee_grouping) {
            ...
            feeGroups.push(<FeeGroup key={groupName} settings={this.props.settings} opIds={feeIds}
                                     title={groupNameText}/>);
        }
