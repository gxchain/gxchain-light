...
function getWitnessOrCommittee(type, acct) {
    ...
    commission_rate = account
    ? account.get("commission_rate") / 10
    : commission_rate;
    return {
        url,
        votes,
    ...
    render() {
      ...
      <tr>
        ...
        <td>
          <div>
            <FormattedAsset
              amount={this.props.showNewVotes ? new_votes : votes}
              asset="1.3.1"
              decimalOffset={5}
              hide_asset={true}
            />
        ...
        <td>
          <button
            ...
            onClick={this.onAction.bind(this, item_id)}
          >
            <Translate content={`account.votes.${this.props.action}_witness`} />
          </button>
        </td>
      </tr>
    );
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th style={{ width: cw[3] }}>
                  <Translate content="account.votes.commission_rate" />
                </th>
                <th style={{ width: cw[4] }}>
                  <Translate content="account.votes.votes" />
                  <Link
                    onClick={() => {
                        this.setState({ showNewVotes: !this.state.showNewVotes });
                    }}
                  >
                    (
                    {this.state.showNewVotes ? (
                      <Translate content="account.votes.see_old_votes" />
                    ) : (
                      <Translate content="account.votes.see_new_votes" />
                    )}
                    )
                  </Link>
                </th>
